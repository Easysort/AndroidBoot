...
if ! tmux has-session -t uploader 2>/dev/null; then
  tmux new-session -d -s uploader "while :; do python upload.py >>'$LOG_DIR/uploader.log' 2>&1; echo 'uploader crashed, restarting in 2s' >>'$LOG_DIR/uploader.log'; sleep 2; done"
fi
